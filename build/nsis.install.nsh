...
Section "Geth" GETH_IDX
  ...
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\bpx-geth.exe" "--http --authrpc.addr localhost --authrpc.port 8551 --authrpc.vhosts localhost"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME} (Testnet).lnk" "$INSTDIR\bpx-geth.exe" "--testnet --http --authrpc.addr localhost --authrpc.port 8551 --authrpc.vhosts localhost"
