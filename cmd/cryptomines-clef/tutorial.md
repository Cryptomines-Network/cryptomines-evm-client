## Initializing Cryptomines-Clef

First things first, Cryptomines-Clef needs to store some data itself. Since that data might be sensitive (passwords, signing rules, accounts), Cryptomines-Clef's entire storage is encrypted. To support encrypting data, the first step is to initialize Cryptomines-Clef with a random master seed, itself too encrypted with your chosen password:

```text
$ cryptomines-clef init
...
The master seed of cryptomines-clef will be locked with a password.
Please specify a password. Do not forget this password!
Password:
Repeat password:

A master seed has been generated into /home/martin/.cryptomines-clef/masterseed.json
...
Cryptomines-Clef is capable of managing both key-file based accounts as well as hardware wallets. To evaluate cryptomines-clef, we're going to point it to our Rinkeby testnet keystore and specify the Rinkeby chain ID for signing (Cryptomines-Clef doesn't have a backing chain, so it doesn't know what network it runs on).

```text
$ cryptomines-clef --keystore ~/.ethereum/rinkeby/keystore --chainid 4
...
INFO [07-01|11:00:46.392] IPC endpoint opened                      url=$HOME/.cryptomines-clef/cryptomines-clef.ipc
------- Signer info -------
* intapi_version : 7.0.0
* extapi_version : 6.0.0
* extapi_http : n/a
* extapi_ipc : $HOME/.cryptomines-clef/cryptomines-clef.ipc
```

By default, Cryptomines-Clef starts up in CLI (Command Line Interface) mode. Arbitrary remote processes may *request* account interactions (e.g. sign a transaction), which the user will need to individually *confirm*.

To test this out, we can *request* Cryptomines-Clef to list all account via its *External API endpoint*:

```text
echo '{"id": 1, "jsonrpc": "2.0", "method": "account_list"}' | nc -U ~/.cryptomines-clef/cryptomines-clef.ipc
```

This will prompt the user within the Cryptomines-Clef CLI to confirm or deny the request:
...
Apart from listing accounts, you can also *request* creating a new account; signing transactions and data; and recovering signatures. You can find the available methods in the Cryptomines-Clef [External API Spec](https://github.com/ethereum/go-ethereum/tree/master/cmd/cryptomines-clef#external-api-1) and the [External API Changelog](https://github.com/ethereum/go-ethereum/blob/master/cmd/cryptomines-clef/extapi_changelog.md).

*Note, the number of things you can do from the External API is deliberately small, since we want to limit the power of remote calls by as much as possible! Cryptomines-Clef has an [Internal API](https://github.com/ethereum/go-ethereum/tree/master/cmd/cryptomines-clef#ui-api-1) too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here.*

## Automatic rules

For most users, manually confirming every transaction is the way to go. However, there are cases when it makes sense to set up some rules which permit Cryptomines-Clef to sign a transaction without prompting the user. One such example would be running a signer on Rinkeby or other PoA networks.
...
Of course, Cryptomines-Clef isn't going to just accept and run arbitrary scripts you give it, that would be dangerous if someone changes your rule file! Instead, you need to explicitly *attest* the rule file, which entails injecting its hash into Cryptomines-Clef's secure store.

```text
$ sha256sum rules.js
645b58e4f945e24d0221714ff29f6aa8e860382ced43490529db1695f5fcc71c  rules.js

$ cryptomines-clef attest 645b58e4f945e24d0221714ff29f6aa8e860382ced43490529db1695f5fcc71c
Decrypt master seed of cryptomines-clef
Password:
...
At this point, we can start Cryptomines-Clef with the rule file:

```text
$ cryptomines-clef --keystore ~/.ethereum/rinkeby/keystore --chainid 4 --rules rules.js
...
INFO [07-01|13:39:49.728] IPC endpoint opened                      url=$HOME/.cryptomines-clef/cryptomines-clef.ipc
------- Signer info -------
* intapi_version : 7.0.0
* extapi_version : 6.0.0
* extapi_http : n/a
...
```text
$ ls -laR ~/.cryptomines-clef/

$HOME/.cryptomines-clef/:
total 24
...
$HOME/.cryptomines-clef/02f90c0603f4f2f60188:
total 12
drwx------ 2 user user 4096 Jul  1 13:25 .
drwxr-x--x 3 user user 4096 Jul  1 13:45 ..
-rw------- 1 user user  159 Jul  1 13:25 config.json

$ cat ~/.cryptomines-clef/02f90c0603f4f2f60188/config.json
...
In `$HOME/.cryptomines-clef`, the `masterseed.json` file was created, containing the master seed. This seed was then used to derive a few other things:

- **Vault location**: in this case `02f90c0603f4f2f60188`.
   - If you use a different master seed, a different vault location will be used that does not conflict with each other (e.g. `cryptomines-clef --signersecret /path/to/file`). This allows you to run multiple instances of Cryptomines-Clef, each with its own rules (e.g. mainnet + testnet).
...
In order to make more useful rules - like signing transactions - the signer needs access to the passwords needed to unlock keys from the keystore. You can inject an unlock password via `cryptomines-clef setpw`.

```text
$ cryptomines-clef setpw 0xd9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3
...
*Note, to make this example work, please use you own accounts. You can create a new account either via Cryptomines-Clef or the traditional account CLI tools. If the latter was chosen, make sure both Cryptomines-Clef and Geth use the same keystore by specifying `--keystore path/to/your/keystore` when running Cryptomines-Clef.*

Attest the new rule file so that Cryptomines-Clef will accept loading it:

```text
$ sha256sum rules.js
f163a1738b649259bb9b369c593fdc4c6b6f86cc87e343c3ba58faee03c2a178  rules.js

$ cryptomines-clef attest f163a1738b649259bb9b369c593fdc4c6b6f86cc87e343c3ba58faee03c2a178
Decrypt master seed of cryptomines-clef
Password:
...
Restart Cryptomines-Clef with the new rules in place:
...
INFO [07-01|14:12:41.638] IPC endpoint opened                      url=$HOME/.cryptomines-clef/cryptomines-clef.ipc
------- Signer info -------
* intapi_version : 7.0.0
* extapi_version : 6.0.0
* extapi_http : n/a
...
```
$ echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0xd9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3", "0x202062617a6f6e6b2062617a2067617a0a"]}' | nc -U ~/.cryptomines-clef/cryptomines-clef.ipc
...
$ echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0xd9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3", "0x2020626f6e6b2062617a2067617a0a"]}' | nc -U ~/.cryptomines-clef/cryptomines-clef.ipc
{"jsonrpc":"2.0","id":1,"error":{"code":-32000,"message":"Request denied"}}
```

Meanwhile, in the Cryptomines-Clef output log you can see:
```text
...
For more details on writing automatic rules, please see the [rules spec](https://github.com/ethereum/go-ethereum/blob/master/cmd/cryptomines-clef/rules.md).

## Geth integration

Of course, as awesome as Cryptomines-Clef is, it's not feasible to interact with it via JSON RPC by hand. Long term, we're hoping to convince the general Ethereum community to support Cryptomines-Clef as a general signer (it's only 3-5 methods), thus allowing your favorite DApp, Metamask, MyCrypto, etc to request signatures directly.

Until then however, we're trying to pave the way via Geth. Geth v1.9.0 has built in support via `--signer <API endpoint>` for using a local or remote Cryptomines-Clef instance as an account backend!

We can try this by running Cryptomines-Clef with our previous rules on Rinkeby (for now it's a good idea to allow auto-listing accounts, since Geth likes to retrieve them once in a while).
...
```text
$ geth --rinkeby --signer=~/.cryptomines-clef/cryptomines-clef.ipc console
...
[{
    accounts: [{
        ...
        url: "extapi://$HOME/.cryptomines-clef/cryptomines-clef.ipc"
    }, {
...
Lastly, when we requested a transaction to be sent, Cryptomines-Clef prompted us in the original window to approve it:
